...
(X, Y) ∈ [0,1]² を 5×5 グリッド（25 ビン）+ 1 ペナルティビン（obs=25；Backspace 8 連続以上）に離散化します。
...
| **β_writing**（あ/カモード） | `true` | 130 | 0.05 | 3.0 | 2.0 | 0.12 |
...
- **タイムアウト：** `make_silence_observation()` が合成沈黙観測値を生成し、`engine.update_silence()` に入力。このメソッドは通常の α=0.30 ではなく**低い α=0.05**（時定数 ≈ 20 秒）で EWMA を更新する。急激な高 Friction 方向へのドリフトを防ぎつつ、長時間サイレンス時に観測ビンが徐々に低 Engagement 方向へ移動し、Incubation 遷移を可能にする。
...
**修正：** ワンショット `has_pending_penalty: Arc<AtomicBool>` フラグを `backspace_streak >= 8` に達した瞬間に設定。フラグは次の `engine.update()` が `atomic swap(false)` で消費するまで保持され、検出と 1 Hz タイマーゲートを分離します。
