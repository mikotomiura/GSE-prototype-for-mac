...
Wall は**スマートフォンベースの Zen タイマー**で解除されます。Wall 発動時、ランダムポートで組み込み HTTP サーバーが起動し、オーバーレイに QR コードが表示されます。スマートフォンでスキャンすると **2 分間のカウントダウンタイマー**が表示され、完了後に「Unlock Wall」ボタンが出現します。「仕事に戻る」早期終了ボタンも常時表示され、即座に解除可能です。PC オーバーレイにはスマホと同期した 120 秒カウントダウンと、スマートフォン接続時に**「Phone connected」バッジ**（`wall-phone-connected` イベント）が表示されます。スマートフォンがない場合は、フォールバックとして **120 秒**後に自動解除されます。
...
| Wall 解除 | Zen タイマー（スマホ 2 分待機） | QR → スマートフォン Zen タイマー → 解除ボタン；スマホなしの場合 150 秒自動解除 |
