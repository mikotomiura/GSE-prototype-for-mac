...
79: ### 処理フロー
80: 1.  **入力検知**: `hook.rs` がシステム全体のキー押下・解放イベントをフック。
81: 2.  **特徴量抽出**: `features.rs` が **6つのマイクロ特徴量 (F1-F6)** を30秒間のスライディングウィンドウで計算:
82:     - **F1 (Median Flight Time)**: 基本的な打鍵速度。
83:     - **F2 (Flight Time Variance)**: リズムの安定性（分散）。
84:     - **F3 (Correction Rate)**: バックスペース・Deleteキーの使用率。
85:     - **F4 (Burst Length)**: 連続した入力（バースト）の平均文字数。
86:     - **F5 (Pause Count)**: 2秒以上の停止回数。
87:     - **F6 (Pause After Delete)**: 修正直後の停止（高い認知負荷の強力なシグナル）。
88: 3.  **HMM 更新**: `engine.rs` がこれらの特徴量から **Stuckスコア (S_stuck)** を算出し、HMMの状態確率を更新します。
89:     - **遷移行列**: 状態のバタつきを抑えるよう再設計。
90:     - **出力行列**: `S_stuck` スコアを観測ビンにマッピング。
91: 4.  **安定性と品質**:
92:     - **Mutex安全性**: `unwrap()` を安全なパターンマッチに置き換え、スレッドパニックによるクラッシュを防止。
93:     - **構造化ログ**: `tracing` クレートを導入し、詳細な診断ログを出力。
