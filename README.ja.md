...
12. [HMM チューニングと安定性（v2.9）](#hmm-チューニングと安定性v29)
13. [ログと分析](#ログと分析)
14. [ビルド手順](#ビルド手順)
15. [学術的参考文献](#学術的参考文献)
...
メインスレッド（Tauri イベントループ + TIS 用 GCD メインキュー）
    ...
    │       │ 合成摩擦: 沈黙 ≥ 8 s → F6/F3 を Stuck 方向へ漸増
...
| 特徴量 | 記号 | 定義 | 認知的意味 |
| --- | --- | --- | --- |
| フライトタイム中央値 | **F1** | キー離し→次キー押下間隔（ms）、30 秒スライディングウィンドウの中央値 | タイピング速度 — 低いほど Flow |
...
α = 0.30  （通常タイピング: update()）
α = 0.15  （沈黙 < 15 秒: update_silence()）
α = 0.25  （深い沈黙 ≥ 15 秒: update_silence()）
```

### 観測ビン

(X, Y) ∈ [0,1]² を 5×5 グリッド（25 ビン）+ 1 ペナルティビン（obs=25；Backspace 14 連続以上）に離散化します。
...
```
π'_j = ( Σ_i  π_i · A[i,j] ) · B[j, obs]    j ∈ {0, 1, 2}
π'   ← π' / Σ_j π'_j                         （正規化）
```

- **A** = 3×3 遷移行列（行 = 遷移元、列 = 遷移先）
- **B** = 3×26 放射行列（状態 × 観測ビン）；全エントリに最小値 0.01 を設定し確率の完全消滅を防止
...
α = 0.40  （通常更新 → 時定数 τ ≈ 2.5 秒）
α = 0.60  （Backspace ペナルティビン → 素早い Stuck 収束）
```

### 修正 ②：確率の離散クラスタリング

**修正：** 全放射テーブルエントリに最小値 **0.01** を設定（ランタイムの加算フロアは廃止）。ペナルティビン（obs=25）の Stuck:Flow 放射比が ~99× となり、人工的な上限効果なしで強力な Stuck シグナルを提供します。
...
```rust
// F6：8 秒超で開始 → 33 秒で 0.50 に到達
F6_synthetic = clamp((silence_secs − 8) / 50,  0.0, 0.50)

// F3：15 秒超で開始 → 47 秒で 0.40 に到達
F3_synthetic = clamp((silence_secs − 15) / 80, 0.0, 0.40)
```

| 沈黙時間 | F3_合成 | F6_合成 | X（摩擦） | x_bin | 領域 |
| --- | --- | --- | --- | --- | --- |
| 8 秒 | 0.00 | 0.00 | ≈ 0.35 | 1 | Incubation |
| 15 秒 | 0.20 | 0.15 | ≈ 0.86 | 4 | **Stuck** |
| 25 秒 | 0.20 | 0.34 | ≈ 1.00 | 4 | **Stuck** |
| 33 秒 | 0.23 | 0.50 | ≈ 1.00 | 4 | **Stuck** |

> **補足：** 沈黙 ≥ 10 秒で摩擦フロアが適用されます（F3 ≥ 0.20, F6 ≥ 0.15）。持続的な沈黙中の Stuck からの自動回復を防止します。F1 = 2000 ms（φ₁ = 1.0）および高い F5 と合わせ、約 15 秒で Stuck 領域に到達します。
...
macOS では、IME 候補ウィンドウの表示状態を CGEventTap コールバック内の**キーストローク駆動状態マシン**（`hook_macos.rs`）で検出します。候補ウィンドウ検出時は `set_paused()` により HMM エンジンを**一時停止**し、確率ベクトルを凍結（リセットではなく IME 前の状態を保持）します。Input Monitoring 以外の追加パーミッションは不要です。
...
| **β_writing**（あ/カモード） | `true` | 100 | 0.10 | 2.5 | 1.0 | 0.12 |
...
分析スレッドは動的な `recv_timeout`（次の 1 秒ゲートまでの残り時間）により **1 Hz タイマーゲート**を実装します。

- **ゲート窓内にキーストローク到着：** 特徴量を蓄積。前回の HMM ステップから 1 秒以上経過した場合のみ `engine.update()` を呼び出す（α = 0.30）。
- **ゲート発火（直近入力なし）：** `make_silence_observation()` が合成沈黙観測値を生成し、`engine.update_silence()` に入力。このメソッドは**低い EWMA α** を使用：沈黙 < 15 秒では α = 0.15（時定数 ≈ 6.6 秒）、深い沈黙 ≥ 15 秒では α = 0.25（時定数 ≈ 4 秒）に動的増加し、Stuck 収束を加速する。
...
| **Lv2** | Wall | p_stuck > 0.70 累積 30 秒（ヒステリシス付き） | 全画面遮断オーバーレイ + メッセージ | 解除まで全入力をブロック |

### Wall 解除：Zen タイマー（v2.7）

Wall は**スマートフォンベースの Zen タイマー**で解除されます。Wall 発動時、ランダムポートで組み込み HTTP サーバーが起動し、オーバーレイに QR コードが表示されます。スマートフォンでスキャンすると **2 分間のカウントダウンタイマー**が表示され、完了後に「Unlock Wall」ボタンが出現します。「仕事に戻る」早期終了ボタンも常時表示され、即座に解除可能です。スマートフォン接続時には PC オーバーレイに**「Phone connected」バッジ**（`wall-phone-connected` イベント）が表示されます。PC 側の自動解除フォールバックはありません — スマートフォンからの解除が必須です。

これは以前の DeviceMotion シェイク検知を置き換えるもので、iOS でのパーミッション要求やデバイス間の信頼性問題を解消しています。
...
### Wall トリガー：ヒステリシス付き累積カウンター（v2.9）

Lv2 Wall は単純な持続時間チェックではなく、**ヒステリシスバンド付き累積ミリ秒カウンター**を使用します。

- **p_stuck > 0.70**：経過時間を 30 秒閾値に向けて累積
- **0.50 ≤ p_stuck ≤ 0.70**：累積を一時停止（スラックバンド — 増減なし）
- **p_stuck < 0.50**：累積を **2 倍速で減衰**（明確な回復シグナル）

閾値付近での短い stuck 振動による早期 Wall 発動を防ぎつつ、持続的な stuck 状態には確実にトリガーします。カウンターは `Date.now()` を使用し、JavaScript タイマーのドリフトに依存しない正確な経過時間を測定します。
...
### Backspace ペナルティ競合条件の修正

**問題：** 高速タイピストが `BS×6 → Enter` と入力した場合、非 BS キーが 1 Hz 分析スレッドの読み取り前に `backspace_streak` を 0 にリセットし、ペナルティが失われる。

**修正：** ワンショット `has_pending_penalty: Arc<AtomicBool>` フラグを `backspace_streak >= 14` に達した瞬間に設定。フラグは次の `engine.update()` が `atomic swap(false)` で消費するまで保持され、検出と 1 Hz タイマーゲートを分離します。
...
ランタイムの加算放射フロア（`EMISSION_FLOOR = 0.05`、`B[j,obs] + ε` として適用）を廃止し、放射テーブル自体に**最小値 0.01 を設定**。ペナルティビンの Stuck:Flow 放射比が ~20× から ~99× に向上し、全状態-観測ペアにゼロでない確率を維持しながら、Backspace 連打時の HMM シグナルを強化します。

### 初期確率の最適化

HMM 初期確率を `[0.50, 0.30, 0.20]` から **`[0.80, 0.15, 0.05]`** に変更。典型的なセッション開始（タイピングは Flow から開始）の期待定常状態と一致させました。EWMA 初期位置も `(0.1, 0.8)`（低摩擦、高エンゲージメント = Flow 領域）に揃え、セッション開始時の 3〜5 秒間の「ウォームアップフリッカー」（確率が収束前に振動する現象）を解消します。

### 表示ヒステリシス α の引き上げ

表示 EMA α を `0.25/0.50` から **`0.40/0.60`** に引き上げ。放射フロア廃止による鋭い放射を補正し、単一ステップのスパイクを防ぎつつ、真の状態変化への追従性を改善します。

### 遷移行列：Flow 自己ループの低減

Flow→Flow を 0.80 から **0.75** に低減（脱出時間 5 s → 4 s）、確率質量を Flow→Incubation（0.13 → 0.17）と Flow→Stuck（0.07 → 0.08）に再配分。「Flow 重力」（特徴量がシフトしても HMM が Flow に留まり続ける傾向）を緩和します。
...
| Wall 解除 | Zen タイマー（スマホ 2 分待機） | QR → スマートフォン Zen タイマー → 解除ボタン；PC 側自動解除なし |
