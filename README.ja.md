...
Wall は**スマートフォンベースの Zen タイマー**で解除されます。Wall 発動時、ランダムポートで組み込み HTTP サーバーが起動し、オーバーレイに QR コードが表示されます。スマートフォンでスキャンすると **2 分間のカウントダウンタイマー**が表示され、完了後に「Unlock Wall」ボタンが出現します。スマートフォンがない場合は、フォールバックとして **150 秒**後に自動解除されます。

これは以前の DeviceMotion シェイク検知を置き換えるもので、iOS でのパーミッション要求やデバイス間の信頼性問題を解消しています。

### Monk Mode（v2.7）

ダッシュボードに **「Monk Mode — Force Break」** ボタンを追加。現在の認知状態に関係なく、`emit("wall-activate")` で手動で Wall を発動できます。ユーザーが能動的に休憩を取るための機能です。
