...
//   The CGEventTap callback in hook_macos.rs maintains a keystroke-driven
//   state machine that tracks IME composition and candidate selection state:
//
//     IME_COMPOSING: true when the user is typing romaji in Japanese mode
//                    (marked/underlined text visible).
//     IME_ACTIVE:    true when the candidate window is visible (after Space
//                    is pressed during composition).
//
//   This approach is:
...
//   - macOS-version-independent: works regardless of how the OS renders
//     the candidate window (CGWindow, CALayer, etc.).
//
//   Previous approaches that were investigated and rejected:
//   - CGWindowListCopyWindowInfo: JapaneseIM-RomajiTyping (macOS 26) does
//     not create CGWindows for the candidate popup.
//   - Accessibility API (AXMarkedTextRange): AXFocusedApplication returns
//     kAXErrorCannotComplete from background threads on macOS 26.
// ---------------------------------------------------------------------------

/// Monitors whether an IME candidate window is currently visible.
///
/// macOS: reads the `IME_ACTIVE` atomic set by the CGEventTap callback's
/// keystroke state machine.  When true, the HMM engine is paused.
pub struct ImeMonitor;

impl ImeMonitor {
    pub fn new() -> Self {
        tracing::info!(
            "IME Monitor: keystroke-based composition detection enabled \
             (CGEventTap state machine)"
        );
        Self
    }

    /// Returns `true` if the IME candidate window is currently visible.
    pub fn is_candidate_window_open(&self) -> bool {
        crate::input::hook::IME_ACTIVE.load(Ordering::Acquire)
    }
}
